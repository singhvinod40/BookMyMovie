...
const NavBar = () => {
  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16
      lg:px-36 py-5'>
    <Link to= '/' className='max-md:flex-1'> 
    <img src={assets.logo} alt='flick' className='w-36 h-auto' />
    </Link>    
    ...
    <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium 
      ...
      <XIcon className='md:hidden absolur top-6 right-6 w-6 h-6 cursor-pointer ' onClick={() =>setIsOpen(!isOpen)}/>
      
      <Link onClick={() => {scrollTo(0,0);setIsOpen(false)}} to='/' >Home</Link>
      <Link onClick={() => {scrollTo(0,0);setIsOpen(false)}} to='/movies' >Movie</Link>
    ...
    <div className='flex items-center gap-8'>
      <SearchIcon className='max-md:hidden w-6 h-6 cusror-pointer'/>
    ...
    <MenuIcon  onClick={() =>setIsOpen(!isOpen)} className='max-md:ml-4 md:hidden w-8 h-8 cusror-pointer'/>
